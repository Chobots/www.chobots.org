(function (window) {
    ...
    function emit(event) {
        observers.forEach(function (listener) {
            try {
                ...
                log("WebSocket observer failed", error);
            }
        });
    }

    function notifyOpen(detail) {
        callSwf("chobotsOnWsOpen", [detail]);
        emit({ type: "open", detail: detail });
    }

    function notifyFrame(frame) {
        callSwf("chobotsOnWsFrame", [frame]);
    ...
    function notifyClose(detail) {
        callSwf("chobotsOnWsClose", [detail]);
    ...
    function closeLocal(transport, code, reason, notify) {
        ...
        if (notify) notifyClose({ code: code || 1000, reason: reason || "client_disconnect" });
    }
    ...
    function connectNetwork(url, retriesRemaining) {
        ...
        nextSocket.onclose = function (event) {
            ...
            if (pendingError) {
                ...
                notifyError(pendingError);
            }
            setTransportStatus("closed", "websocket");
            notifyClose({ code: event.code, reason: event.reason });
    ...
    window.chobotsObserveWs = function (listener) {
        if (typeof listener !== "function") throw new TypeError("WebSocket observer must be a function");
        observers.add(listener);
        return function () { observers.delete(listener); };
    };
})(window);
