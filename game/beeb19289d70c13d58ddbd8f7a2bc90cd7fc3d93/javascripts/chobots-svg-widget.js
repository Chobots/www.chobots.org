(function (root, factory) {
    ...
    function validateSvg(root, bytes) {
        ...
        Array.from(svg.querySelectorAll("*")).concat([svg]).forEach(function (element) {
            Array.from(element.attributes || []).forEach(function (attribute) {
                ...
                if (name.startsWith("on") || /(?:https?:|javascript:|data:text\/html)/i.test(value) || (/url\(/i.test(value) && !/url\(\s*['"]?#/i.test(value))) throw stableError("invalid_svg");
            });
        });
    ...
    return { createController: createController, resolveRequest: resolveRequest };
});
